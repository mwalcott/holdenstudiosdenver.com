...
class VPC_Updating_Manager {
	 ...
	 * @var string
	 */
	protected $url = 'http://codecanyon.net/item/woocommerce-visual-products-configurator/9058551?ref=orionorigin';
	/**
...
//		}
	...
	public function addUpgradeMessageLink() {
            ...
            $username = $wpc_options_settings["envato-username"];
            $api_key = $wpc_options_settings["envato-api-key"];
            $purchase_code = $wpc_options_settings["purchase-code"];
            ...
            if ( empty( $username ) || empty( $api_key ) || empty( $purchase_code ) ) {
                    echo ' <a href="' . $this->url . '">' . __( 'Download new version from CodeCanyon.', 'vpc' ) . '</a>';
            } else {
